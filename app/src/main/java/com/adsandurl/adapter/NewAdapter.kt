...
class NewAdapter(
    private var listOfHotData: List<NewChildren>,
    ...
    private var mFilteredList: List<NewChildren>? = null
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ItemViewHolder) {
            holder.tvName.setText(getItem(position).getChildrenObject()!!.getName()!!)
            holder.tvTitle.setText(getItem(position).getChildrenObject()!!.getTitle()!!)
            Glide.with(context).load(getItem(position).getChildrenObject()!!.getThumbnail()).into(holder.ivThumbnail);
    ...
    override fun getFilter(): Filter {
        return object : Filter() {
            ...
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                val results = ArrayList<NewChildren>()
                ...
                if (constraint != null) {
                    if (mFilteredList != null && mFilteredList!!.size > 0) {
                        for (newChildren in mFilteredList!!) {
                            if (newChildren.getChildrenObject()!!.getTitle()!!.toLowerCase().contains(
                                    constraint.toString().toLowerCase()
                                ) || newChildren.getChildrenObject()!!.getName()!!.toLowerCase().contains(
                                    constraint.toString().toLowerCase()
                                )
                            )
                                results.add(newChildren)
                        }
                    }
            ...
            override fun publishResults(constraint: CharSequence, results: FilterResults) {
                listOfHotData = results.values as List<NewChildren>
    ...
    fun getItem(position: Int): NewChildren {
    ...
    fun setList(list: List<NewChildren>) {
